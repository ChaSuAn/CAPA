...
if __name__ == '__main__':
    batch_size, max_seq_len = 32, 512  # Damn loss limition
    Max_epoch = 150
    train_iter, test_iter, train_batch_count, test_batch_count = load_data('contrast_train_test_20com.xlsx',
    ...
    for epoch in range(Max_epoch):
        ...
        for step, batch_data in tqdm(enumerate(train_iter), desc='train epoch:{}/{}'.format(epoch + 1, Max_epoch)
                , total=train_batch_count):

            batch_list = batch_data[:21]
            ...
            batch_labels = torch.tensor(batch_data[21].to(device))
            ...
            # Printing the mean and variance of custom weights
            # print(f"Epoch {epoch + 1}")
            # print("Custom Weights:")
            # for name, param in model.named_parameters():
            #     if "w_" in name or "b_" in name:  # 只打印自定义权重
            #         print(f"{name} - Mean: {param.data.mean().item()}, Std: {param.data.std().item()}")
            #         print(param.data)
            #         diy_weight = str(name) + 'Mean:' + str(param.data.mean().item()) + 'Std:' + str(param.data.std().item())
            #         DiyWeight.append(diy_weight)

            # Print the mean and variance of the custom weight gradient
            # print("Custom Gradients:")
            # for name, param in model.named_parameters():
            #     if param.grad is not None and ("w_" in name or "b_" in name):  # 只打印自定义权重的梯度
            #         print(f"{name} - Mean: {param.grad.mean().item()}, Std: {param.grad.std().item()}")
            #         diy_weight_grad = str(name) + 'Mean:' + str(param.grad.mean().item()) + 'Std:' + str(param.grad.std().item())
            #         DiyWeightGrad.append(diy_weight_grad)

            # Checking the paradigm of the gradient
            # total_norm = 0.0
            ...
            # GradNorm.append(str(total_norm))

        model.eval()
        LOSS.append(train_loss_sum)
        ...
        print(result)
    torch.save(model, 'Model_CAPC.bin')
